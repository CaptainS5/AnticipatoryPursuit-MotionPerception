...
%         perceptualIdx(deleteI{subN}) = [];
    ...
    if individualPlots==1
        cd(perceptFolder)
        drawPlot(subN, nodesPermeanSub, nodesPerstdSub, 'Trial', 'Choice-coh', probNameSub, 'treePlotPerception_Exp1_', names{subN}, [])
        % pursuit plots
        cd(pursuitFolder)
        % anticipatory pursuit
        drawPlot(subN, nodesPmeanSub, nodesPstdSub, 'Trial', 'Anticipatory pursuit velocity (deg/s)', probNameSub, 'treePlotPursuit_Exp1_', names{subN}, [])
%         close all
    end
end
...
if averagedPlots==1
    % perception plots
    cd(perceptFolder)
    drawPlot(1, nodesPermean, nodesPerste, 'Trial', 'Choice-coh', probNames{2}, 'treePlotPerception_Exp1_', 'all', [])
    
    % pursuit plots
    cd(pursuitFolder)
    % anticipatory pursuit
    drawPlot(1, nodesPmean, nodesPste, 'Trial', 'Anticipatory pursuit velocity (deg/s)', probNames{2}, 'treePlotPursuit_Exp1_', 'all', [])
end
