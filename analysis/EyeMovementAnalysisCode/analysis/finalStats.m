...
for expN = 1:3
    ...
    for subN = 1:size(dataAll{expN}.eyeTrialData.sub, 1)
        excludeN(subN, 1) = length(find(dataAll{expN}.eyeTrialData.errorStatus(subN, :)~=0 & dataAll{expN}.eyeTrialData.trialType(1, :)==0));
    end
    excludeRatio = excludeN/length(find(dataAll{expN}.eyeTrialData.trialType(1, :)==0));
