        ...
        for probSubN = 1:size(probSub, 2)
            ...
            for slideI = 1:(length(idxT)-trialBin+1)
                if strcmp(checkParas{paraN}, 'choice')
                    ...
                    yValuesAll{paraN, subN}(probSubN, slideI) = nanmean(eyeTrialData.choice(subN, idxT(slideI:(slideI+trialBin-1))))-probRight;
                else
            ...
            for slideI = 1:(length(idxT)-trialBin+1)
                if strcmp(checkParas{paraN}, 'choice')
                    probRightNum = length(find(eyeTrialData.rdkDir(subN, idxT(slideI:(slideI+trialBin-1)))>0));
                    probRight = probRightNum/trialBin;
                    yValuesS{paraN, subN}(probSubN, slideI) = nanmean(eyeTrialData.choice(subN, idxT(slideI:(slideI+trialBin-1))))-probRight;
                else
            ...
            for slideI = 1:(length(idxTL)-trialBin+1)
                if strcmp(checkParas{paraN}, 'choice')
                    probRight = 0;
                    yValuesSL{paraN, subN}(probSubN, slideI) = nanmean(eyeTrialData.choice(subN, idxTL(slideI:(slideI+trialBin-1))))-probRight;
                else
            ...
            for slideI = 1:(length(idxTR)-trialBin+1)
                if strcmp(checkParas{paraN}, 'choice')
                    probRight = 1;
                    yValuesSR{paraN, subN}(probSubN, slideI) = nanmean(eyeTrialData.choice(subN, idxTR(slideI:(slideI+trialBin-1))))-probRight;
                else
            ...
            for slideI = 1:(length(idxT)-trialBin+1)
                if strcmp(checkParas{paraN}, 'choice')
                    probRightNum = length(find(eyeTrialData.rdkDir(subN, idxT(slideI:(slideI+trialBin-1)))>0));
                    probRight = probRightNum/trialBin;
                    yValuesP{paraN, subN}(probSubN, slideI) = nanmean(eyeTrialData.choice(subN, idxT(slideI:(slideI+trialBin-1))))-probRight;
                else
            ...
            for slideI = 1:(length(idxTL)-trialBin+1)
                if strcmp(checkParas{paraN}, 'choice')
                    probRight = 0;
                    yValuesPL{paraN, subN}(probSubN, slideI) = nanmean(eyeTrialData.choice(subN, idxTL(slideI:(slideI+trialBin-1))))-probRight;
                else
            ...
            for slideI = 1:(length(idxTR)-trialBin+1)
                if strcmp(checkParas{paraN}, 'choice')
                    probRight = 1;
                    yValuesPR{paraN, subN}(probSubN, slideI) = nanmean(eyeTrialData.choice(subN, idxTR(slideI:(slideI+trialBin-1))))-probRight;
                else
