...
yRange = [0 0.4];
chooseTrialN = 126; % how many trials in each sorted group
...
subNexp = zeros(1, 3);
for subN = 1:size(nameSets{1}, 2)
    if ~strcmp(nameSets{2}{subN}, 'nan')% has data for exp2
        subNexp(1) = subN; % for exp 1
        subNexp(2) = subNexp(2)+1; % for exp 2, fixation control
        subNexp(3) = subN; % for exp3
    end
    
    probSub = unique(expAll{2}.eyeTrialData.prob(subNexp(2), expAll{2}.eyeTrialData.errorStatus(subNexp(2), :)==0));
    if probSub(1)<50
        probNameI = 1;
...
%             hold on
            ...
            for probSubN = 1:length(probSub)
                ...
                % sort the asp values
                trialIdx{subNexp(2), expN}{probSubN} = idxP;
                choiceAll{subNexp(2), expN}{probSubN} = expAll{expN}.eyeTrialData.choice(subNexp(expN), idxP);
                cohAll{subNexp(2), expN}{probSubN} = expAll{expN}.eyeTrialData.coh(subNexp(expN), idxP);
                aspAll{subNexp(2), expN}{probSubN} = expAll{expN}.eyeTrialData.pursuit.APvelocityX(subNexp(expN), idxP);
                
%                 % plot the distributions
%                 p{subNexp(2), expN}{probSubN} = histogram(aspAll{subNexp(2), expN}{probSubN}, edges, 'Normalization','probability');
%                 if probSubN==1
%                     line([nanmean(aspAll{subNexp(2), expN}{probSubN}) nanmean(aspAll{subNexp(2), expN}{probSubN})], yRange, 'linestyle', '--', 'color', 'b')
%                 else
%                     line([nanmean(aspAll{subNexp(2), expN}{probSubN}) nanmean(aspAll{subNexp(2), expN}{probSubN})], yRange, 'linestyle', '--', 'color', 'r')
%                 end
            end
%             xlabel(['exp', num2str(expN), ' asp'])
%             ylim(yRange)
%             legend([p{subNexp(2), expN}{:}], probNames{probNameI})
        end
    end
%     cd(pursuitFolder)
%     saveas(gcf, ['aspDistribution_', names2{subNexp(2)}, '.pdf'])
end
...
%     fig3 = figure;
    ...
    for expN = 1:2
        ...
        if expN==1
            ...
            leftI1 = aspI1(chooseTrialN+1:end);
            leftI2 = aspI2(1:end-chooseTrialN);
        elseif expN==2
            % for exp3, take the most similar groups
            tempI1 = medI1(1:chooseTrialN);
            tempI2 = medI2(1:chooseTrialN);
            leftI1 = aspI1(chooseTrialN+1:end);
            leftI2 = aspI2(chooseTrialN+1:end);
        end
        leftIdx = [trialIdx{subN, expN}{1}(leftI1) trialIdx{subN, expN}{2}(leftI2)];
        trialTypeCleaned{expN}(subN, leftIdx) = 3;
        aspSorted{subN, expN}(:, 1) = tempASP1(tempI1);
        aspSorted{subN, expN}(:, 2) = tempASP2(tempI2);
        trialISorted{subN, expN}(:, 1) = trialIdx{subN, expN}{1}(tempI1);
        trialISorted{subN, expN}(:, 2) = trialIdx{subN, expN}{2}(tempI2);
        cohSorted{subN, expN}(:, 1) = cohAll{subN, expN}{1}(tempI1);
        cohSorted{subN, expN}(:, 2) = cohAll{subN, expN}{2}(tempI2);
        choiceSorted{subN, expN}(:, 1) = choiceAll{subN, expN}{1}(tempI1);
        choiceSorted{subN, expN}(:, 2) = choiceAll{subN, expN}{2}(tempI2);
        
%         % plot asp distributions
%         figure(fig1)
%         subplot(2, 1, expN)
%         hold on
%         p1{1} = histogram(aspSorted{subN, expN}(:, 1), edges, 'Normalization','probability');
%         p1{2} = histogram(aspSorted{subN, expN}(:, 2), edges, 'Normalization','probability');
%         line([nanmean(aspSorted{subN, expN}(:, 1)) nanmean(aspSorted{subN, expN}(:, 1))], yRange, 'linestyle', '--', 'color', 'b')
%         line([nanmean(aspSorted{subN, expN}(:, 2)) nanmean(aspSorted{subN, expN}(:, 2))], yRange, 'linestyle', '--', 'color', 'r')
%         ylim(yRange)
%         xlabel(['exp', num2str(expN), ' asp'])
...
%         subplot(2, 1, expN)
%         hold on
%         while trialISorted{subN, expN}(1, 1)>682
%             trialISorted{subN, expN}(:, 1) = trialISorted{subN, expN}(:, 1)-682;        
%         end
%         while trialISorted{subN, expN}(1, 2)>682
%             trialISorted{subN, expN}(:, 2) = trialISorted{subN, expN}(:, 2)-682;        
%         end
%         p2{1} = histogram(mod(682, trialISorted{subN, expN}(:, 1)-1)+1, edgesIdx);
%         p2{2} = histogram(mod(682, trialISorted{subN, expN}(:, 2))+1, edgesIdx);
...
%         subplot(2, 1, expN)
%         hold on
%         p3{1} = histogram(cohSorted{subN, expN}(:, 1), edgesCoh);
%         p3{2} = histogram(cohSorted{subN, expN}(:, 2), edgesCoh);
%         xlabel(['exp', num2str(expN), ' trial coh'])
%         legend([p3{:}], probNames{probNameI})
%         hold off
    end
%     cd(pursuitFolder)
%     saveas(fig1, ['aspDistributionSorted' num2str(chooseTrialN) '_', names2{subN}, '.pdf'])
%     saveas(fig2, ['temporalDistributionSorted' num2str(chooseTrialN) '_', names2{subN}, '.pdf'])
%     saveas(fig3, ['cohDistributionSorted' num2str(chooseTrialN) '_', names2{subN}, '.pdf'])
end
% saving the cleaned data... mostly for velocity traces plotting
cd(analysisFolder)
...
% hold on
% for subN = 1:size(dataPercept.alpha, 1)
%     fS{subN, 1} = plot(dataPercept.alpha(subN, 2)-dataPercept.alpha(subN, 1), aspMean(subN, 2)-aspMean(subN, 1), '.', 'color', markerC(subN, :), 'markerSize', 25);
%     fS{subN, 2} = plot(dataPercept.alpha(subN, 4)-dataPercept.alpha(subN, 3), aspMean(subN, 4)-aspMean(subN, 3), '^', 'MarkerFaceColor', markerC(subN, :), 'MarkerEdgeColor', 'none', 'markerSize', 8);
% end
% axis square
% xlabel('PSE diff')
% ylabel('ASP diff')
% legend([fS{1, :}], {'exp1', 'exp3'}, 'box', 'off', 'location', 'northwest')
% ah1=axes('position',get(gca,'position'), 'visible', 'off');
% legend(ah1, [fS{:, 1}], names, 'box', 'off', 'location', 'northeast')
% cd(perceptFolder)
% saveas(gcf, ['PSEvsASPdiff_exp1vs3sorted_all' num2str(chooseTrialN) '.pdf'])
