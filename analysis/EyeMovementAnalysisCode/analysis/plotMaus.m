...
% load([mausFolder '\previous perceived motion\percept_AP_binMax5.mat'])
load('percept_AP_randomOrder_binMax5.mat')
...
% or visual motion
% load([mausFolder '\previous visual motion\percept_AP_binMax5.mat'])
...
aveWinAP(40) = 1; % change the ones you want to plot to 1
perceptPlots = 1;
APplots = 1;

cohLevels = unique(eyeTrialData.coh(1, eyeTrialData.trialType(1, :)==0))';

%% check for the difference between visual and perceived motion?
% visualD = load([mausFolder '\previous visual motion\percept_AP_binMax5.mat']);
% perceivedD = load([mausFolder '\previous perceived motion\percept_AP_binMax5.mat']);
% windowSize = 15;
% 
% cd(mausFolder)
...
%         hold on
%         plot(visualD.motionDprevious{windowSize, probNmerged}{subN}, '-k')
%         plot(perceivedD.motionDprevious{windowSize, probNmerged}{subN}, '--k')
%         plot(perceivedD.perceptRate{windowSize, probNmerged}{subN},'--r')
%         legend({'visual' 'perceived' 'proportion of perceptual trials'}, 'box', 'off', 'location', 'best')
%         title(['prob' num2str(probCons(probNmerged+2))])
%         xlabel('Trial')
%         ylabel('previous mean direction')
%     end
%     saveas(gcf, ['motionDprevious_windowSize' num2str(windowSize) '_' names{subN} '.pdf'])
% end
...
if perceptPlots==1
    for windowSize = 1:wMax
        if aveWinP(windowSize)==1
            ...
            % plot averaged proportions in each probability
            for probNmerged = 1:3
                figure
                hold on
                ...
                xD(idxNaN) = NaN;
                plot(nanmean(xD), ...
                    nanmean(dataM.percept.propotionMoreRight{windowSize, probNmerged}), ...
                    'o-', 'Color', colorProb(probNmerged+2, :), 'LineWidth', 2)
                for subN = 1:length(names)
                    plot(previousDbins{windowSize, probNmerged}(subN, :), ...
                        dataM.percept.propotionMoreRight{windowSize, probNmerged}(subN, :), '--o')
                end
                title(['windowSize ' num2str(windowSize) ', prob ' num2str(probCons(probNmerged+2))])
                saveas(gcf, ['proportionMoreRightVSpreviousMeanD_binMax' num2str(binMax) '_prob' num2str(probCons(probNmerged+2)) '_' num2str(windowSize) 'trials.pdf'])
            end
...
%             end
    ...
    hold on
    for probNmerged = 1:3
        p{probNmerged} = plot(1:wMax, corrM.percept.rho(:, probNmerged), 'Color', colorProb(probNmerged+2, :))
        idxSig = find(corrM.percept.pValue(:, probNmerged)<0.05);
        plot(idxSig, corrM.percept.rho(idxSig, probNmerged), '*', 'Color', colorProb(probNmerged+2, :))
    end
    legend([p{:}], probNames{2}, 'box', 'off', 'location', 'best')
    xlabel('Trial history length')
    ylabel('Correlation (proportion of right bias)')
    saveas(gcf, ['correlationVStrialHistoryLength_perception_binMax' num2str(binMax) '.pdf'])

%% plot sum Diff vs window size
%     figure
%     hold on
%     for probNmerged = 1:3
%         p{probNmerged} = plot(1:wMax, sumDiff.percept.mean(:, probNmerged), 'Color', colorProb(probNmerged+2, :))
%         for windowSize = 1:wMax
%             steTemp(windowSize) = nanstd(sumDiff.percept.sub{windowSize}(:, probNmerged))/sqrt(length(names));
%         end
%         errorbar(1:wMax, sumDiff.percept.mean(:, probNmerged), steTemp, 'Color', colorProb(probNmerged+2, :))
%     end
%     legend([p{:}], probNames{2}, 'box', 'off', 'location', 'best')
%     xlabel('Trial history length')
%     ylabel('Sum difference')
%     saveas(gcf, ['sumDiffVStrialHistoryLength_perception_binMax' num2str(binMax) '.pdf'])
end
% close all
...
%             
            ...
            for probNmerged = 1:3
                ...
                xD = previousDbins{windowSize, probNmerged};
                xD(idxNaN) = NaN;
                p{probNmerged} = plot(nanmean(xD), ...
                    nanmean(dataM.AP.proportionRight{windowSize, probNmerged}), '-o', ...
                    'Color', colorProb(probNmerged+2, :), 'LineWidth', 2);
%                 lsline
                for subN = 1:length(names)
                    hold on
                    s{subN} = plot(previousDbins{windowSize, probNmerged}(subN, :), ...
                        dataM.AP.proportionRight{windowSize, probNmerged}(subN, :), '--o');
                end
                legend([p{:}], probNames{2}, 'box', 'off', 'location', 'northeastoutside')
                legend([s{:}], names, 'box', 'off', 'location', 'northeastoutside')
                axis square
...
%                 end
    ...
    for probNmerged = 1:3
        ...
        idxSig = find(corrM.AP.pValue(:, probNmerged)<0.05);
        plot(idxSig, corrM.AP.rho(idxSig, probNmerged), '*', 'Color', colorProb(probNmerged+2, :))
    end
...
%     hold on
%     for probNmerged = 1:3
%         p{probNmerged} = plot(1:wMax, sumDiff.AP.mean(:, probNmerged), 'Color', colorProb(probNmerged+2, :))
%         for windowSize = 1:wMax
%             steTemp(windowSize) = nanstd(sumDiff.AP.sub{windowSize}(:, probNmerged))/sqrt(length(names));
%         end
%         errorbar(1:wMax, sumDiff.AP.mean(:, probNmerged), steTemp, 'Color', colorProb(probNmerged+2, :))
%     end
%     legend([p{:}], probNames{2}, 'box', 'off', 'location', 'best')
%     xlabel('Trial history length')
%     ylabel('Sum difference')
%     saveas(gcf, ['sumDiffVStrialHistoryLength_AP_binMax' num2str(binMax) '.pdf'])
end